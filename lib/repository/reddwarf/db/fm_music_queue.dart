...
class FmMusicQueue {
  ...
  static Future<int> getFmCachedMusicCount() async {
    ...
    final cachedMusicCount =
        await db.rawQuery('SELECT COUNT (*) from fm_queue');
  ...
  static Future<FmMusic?> getFmMusic() async {
    ...
    List<Map<String, dynamic>> maps =
        await db.query('fm_queue', orderBy: "createdtime asc", limit: 1);
    if (maps.isNotEmpty) {
  ...
  static Future<FmMusic?> getFmMusicById(int id) async {
    final db = await initialDatabase();
    List<Map<String, dynamic>> maps =
        await db.query('fm_queue', where: 'id = ?', whereArgs: [id], limit: 1);
    if (maps.isNotEmpty) {
      return FmMusic.fromJson(maps[0]);
    }
    return Future.value(null);
